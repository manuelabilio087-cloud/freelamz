    ...
    <>
      <style>{`
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .sidebar{width:240px;background:${surf};border-right:1px solid ${bord};display:flex;flex-direction:column;position:fixed;top:0;left:0;height:100vh;z-index:50}
        .sb-logo{padding:18px 16px 14px;border-bottom:1px solid ${bord};display:flex;align-items:center;gap:10px;cursor:pointer}
        .logo-mark{width:30px;height:30px;background:${acc};border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
        .logo-txt{font-size:16px;font-weight:700;color:${txt}}
        .logo-txt span{color:${acc}}
        .sb-nav{flex:1;overflow-y:auto;padding:10px 8px}
        .sec-lbl{font-size:10px;font-weight:600;color:${sub};text-transform:uppercase;letter-spacing:.8px;padding:8px 8px 4px}
        ...
        .sb-bot{padding:10px 8px;border-top:1px solid ${bord}}
        ...
        .av{width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,${acc},${pro});display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:700;flex-shrink:0;overflow:hidden}
        .av img{width:100%;height:100%;object-fit:cover}
        .u-name{font-size:13px;font-weight:600;color:${txt}}
        .u-role{font-size:11px;color:${sub}}
        .main{margin-left:240px;flex:1;min-height:100vh;background:${bg}}
        .topbar{background:${surf};border-bottom:1px solid ${bord};padding:0 24px;height:58px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:40}
        .pg-title{font-size:15px;font-weight:600;color:${txt}}
        .tb-right{display:flex;align-items:center;gap:8px}
        .ic-btn{width:32px;height:32px;border-radius:8px;border:1px solid ${bord};background:${surf2};display:flex;align-items:center;justify-content:center;cursor:pointer;color:${sub};transition:all .15s}
        ...
        .cont{padding:22px 24px}
        .plan-box{background:${surf};border:1px solid ${isPro ? pro : bord};border-radius:12px;padding:16px 20px;display:flex;align-items:center;gap:16px;margin-bottom:20px}
        .plan-ic{width:40px;height:40px;border-radius:10px;background:${proB};display:flex;align-items:center;justify-content:center;color:${proT};flex-shrink:0}
        .plan-inf{flex:1}
        .plan-tit{font-size:13.5px;font-weight:600;color:${txt};margin-bottom:2px}
        .plan-sub{font-size:12px;color:${sub};margin-bottom:6px}
        ...
        .plan-meta{display:flex;justify-content:space-between;font-size:11px;color:${sub};margin-top:3px}
        .btn-up{background:${pro};color:#fff;padding:7px 14px;border-radius:8px;font-size:13px;font-weight:600;border:none;cursor:pointer;white-space:nowrap}
        .s-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px}
        .s-card{background:${surf};border:1px solid ${bord};border-radius:12px;padding:18px;transition:all .2s}
        .s-card:hover{transform:translateY(-2px);box-shadow:0 6px 20px rgba(0,0,0,${dark?.12:.06})}
        .s-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}
        .s-icon{width:34px;height:34px;border-radius:8px;display:flex;align-items:center;justify-content:center}
        .s-lbl{font-size:12px;color:${sub};font-weight:500}
        .s-val{font-size:24px;font-weight:700;color:${txt};margin-bottom:3px;line-height:1}
        .s-sub{font-size:12px;color:${sub};display:flex;align-items:center;gap:3px}
        .s-up{color:${grn}}
        .prog{height:3px;background:${bord};border-radius:2px;overflow:hidden;margin-top:8px}
        ...
        .sec-tit{font-size:14px;font-weight:600;color:${txt}}
        ...
        .q-card{background:${surf};border:1px solid ${bord};border-radius:10px;padding:14px;cursor:pointer;text-align:center;transition:all .15s}
        .q-card:hover{border-color:${acc};transform:translateY(-1px)}
        .q-icon{width:38px;height:38px;border-radius:10px;display:flex;align-items:center;justify-content:center;margin:0 auto 8px}
        .q-lbl{font-size:12.5px;font-weight:500;color:${txt}}
        .p-list{display:flex;flex-direction:column;gap:8px}
        .p-row{background:${surf};border:1px solid ${bord};border-radius:10px;padding:13px 15px;display:flex;align-items:center;gap:10px;cursor:pointer;transition:all .15s}
        .p-row:hover{border-color:${acc}}
        .p-cat{width:34px;height:34px;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
        .p-inf{flex:1;min-width:0}
        .p-name{font-size:13.5px;font-weight:600;color:${txt};margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        ...
        .p-bud{font-size:13.5px;font-weight:700;color:${txt}}
        .empty{text-align:center;padding:40px;background:${surf};border:1px solid ${bord};border-radius:12px;color:${sub};font-size:14px}
        .btn-pri{background:${acc};color:#fff;padding:9px 18px;border-radius:8px;font-size:13.5px;font-weight:600;border:none;cursor:pointer;margin-top:12px}
        .prof-card{background:${surf};border:1px solid ${bord};border-radius:12px;padding:28px;max-width:440px;margin:0 auto;text-align:center}
        .prof-av{width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,${acc},${pro});display:flex;align-items:center;justify-content:center;color:#fff;font-size:28px;font-weight:700;margin:0 auto 14px;overflow:hidden}
        .prof-nm{font-size:20px;font-weight:700;color:${txt};margin-bottom:3px}
        .prof-em{font-size:13px;color:${sub};margin-bottom:18px}
        .prof-sts{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;background:${surf2};border-radius:10px;padding:14px;margin-bottom:18px}
        .ps-val{font-size:20px;font-weight:700;color:${accT}}
      ...
      <div className="shell">
        <aside className="sidebar">
          <div className="sb-logo" onClick={() => router.push("/")}>
            ...
            <span className="logo-txt">Freel<span>amz</span></span>
          </div>
              ...
              <div key={sec.label}>
                ...
                {sec.items.map(it => (
                  <div key={it.id} className={`nav-it ${tab === it.id ? "active" : ""}`} onClick={() => navGo(it.id)}>
                    {it.icon} {it.label}
                    {(it as any).badge > 0 && <span className="nav-bdg">{(it as any).badge}</span>}
                  </div>
                ))}
              </div>
            ))}
          ...
          <div className="sb-bot">
            <div className="nav-it" onClick={toggleDark} style={{marginBottom:"2px"}}>
              {dark ? <Sun/> : <Moon/>} {dark ? "Tema claro" : "Tema escuro"}
            </div>
            <div className="nav-it" onClick={logout}><Out/> Sair</div>
            <div className="user-row" style={{marginTop:"6px"}} onClick={() => navGo("profile")}>
              <div className="av">{user?.avatar ? <img src={user.avatar} alt=""/> : user?.name?.[0]}</div>
              <div>
                <div className="u-name">{user?.name}</div>
                <div className="u-role">{isPro ? "Freelancer Pro" : "Freelancer"}</div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <main className="main">
          <div className="topbar">
            <div className="pg-title">
              {tab === "overview" ? `Ola, ${user?.name?.split(" ")[0] || "Freelancer"}` : tab === "projects" ? "Projectos" : tab === "proposals" ? "Propostas" : "Dashboard"}
            </div>
            <div className="tb-right">
              {isPro && <div className="pro-pill"><Star/> Pro</div>}
              <div className="ic-btn" onClick={() => router.push("/messages")}><Msg/></div>
              <div className="ic-btn" onClick={() => navGo("profile")}><Sett/></div>
            </div>
          </div>
            ...
            {tab === "overview" && <>
              <div className="plan-box">
                ...
                <div className="plan-inf">
                  <div className="plan-tit">{isPro ? "Plano Pro — propostas ilimitadas" : `Plano Gratuito — ${pUsed}/${pLimit} propostas usadas`}</div>
                  <div className="plan-sub">{isPro ? "Obrigado por seres Pro!" : "200 MT/mes para propostas ilimitadas"}</div>
                  {!isPro && <>
                    <div className="plan-bar"><div className="plan-fill" style={{width:`${Math.min((pUsed/pLimit)*100,100)}%`}}/></div>
                    <div className="plan-meta"><span>{pUsed} usadas</span><span>{pLimit-pUsed} restantes</span></div>
                  </>}
                </div>
              ...
              <div className="s-grid">
                <div className="s-card">
                  ...
                  <div className="s-sub"><span style={{color:sub}}>Aceites:</span> <span style={{color:grn}}>{stats?.accepted || 0}</span></div>
                ...
                <div className="s-card">
                  ...
                  <div className="s-sub" style={{display:"flex",gap:"2px"}}>
                    {[1,2,3,4,5].map(i => <svg key={i} width="12" height="12" viewBox="0 0 24 24" fill={i<=Math.round(stats?.rating||0)?amb:"none"} stroke={amb} strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>)}
                  </div>
                </div>
              </div>
              ...
              <div className="q-grid">
                {[
                  {l:"Mensagens",i:<Msg/>,p:"/messages",bg:accB,c:accT},
              ...
              <div className="p-list">
                {loading ? (
                  ...
                  <div className="empty">
                    <p>Nenhum projecto disponivel ainda.</p>
                    <button className="btn-pri" onClick={() => router.push("/projects")}>Ver projectos</button>
                  </div>
                ) : projects.slice(0,4).map((p,i) => {
                  const cs = CAT_STYLE[p.category] || {icon:<Brief/>,bg:surf2,col:sub};
                  return (
                    <div key={i} className="p-row" onClick={() => router.push(`/projects/${p.id}`)}>
                      ...
                      <div className="p-inf">
                        ...
                        <div className="p-meta">{p.category||"Geral"} · {p.client_name||"Cliente"}</div>
                      </div>
              ...
              <div className="fade">
                ...
                <div className="p-list">
                  {loading ? <div className="empty">A carregar...</div> : projects.length === 0 ? (
                    <div className="empty"><p>Nenhum projecto disponivel.</p><button className="btn-pri" onClick={() => router.push("/projects")}>Explorar projectos</button></div>
                  ) : projects.map((p,i) => {
                    const cs = CAT_STYLE[p.category] || {icon:<Brief/>,bg:surf2,col:sub};
                    return (
                      <div key={i} className="p-row" onClick={() => router.push(`/projects/${p.id}`)}>
            ...
            {tab === "profile" && (
              <div className="fade">
                <div className="prof-card">
                  <div className="prof-av">{user?.avatar ? <img src={user.avatar} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/> : user?.name?.[0]}</div>
