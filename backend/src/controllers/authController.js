...
const register = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      ...
      process.env.JWT_SECRET || 'freelamz_secret',
      { expiresIn: '7d' }
    );
...
const adminLogin = async (req, res) => {
  ...
  try {
    ...
    if (!user.rows[0].is_admin) {
      return res.status(403).json({ message: 'Acesso negado. Sem privilegios de admin.' });
    }
    const validPassword = await bcrypt.compare(password, user.rows[0].password);
    if (!validPassword) {
...
const forgotPassword = async (req, res) => {
  ...
  try {
    const user = await pool.query('SELECT * FROM users WHERE email = $1', [email]);
    if (user.rows.length === 0) {
      return res.json({ message: 'Se este email estiver registado, enviaremos instrucoes.' });
    }
    ...
    res.json({ message: 'Se este email estiver registado, enviaremos instrucoes.' });
  } catch (err) {
...
const sendEmailCode = async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `<div style="font-family:sans-serif;text-align:center;padding:32px;"><h1>Freelamz</h1><h2>Codigo de verificacao</h2><div style="font-size:40px;font-weight:bold;letter-spacing:12px;color:#6366f1;">${code}</div><p>Valido por 10 minutos.</p></div>`
    });
